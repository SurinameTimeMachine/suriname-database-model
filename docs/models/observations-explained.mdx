...
The Almanakken v2 CSV contains 22,482 rows. Each row is an annual snapshot of a plantation organization: who owned it, who managed it, how many enslaved people were recorded, what it produced, whether it was deserted. The same organization (`wd:Q4392658`, Geijersvlijt) appears in row after row, year after year, with different values.
...
The Almanakken columns are annual, source-qualified evidence. They do not by
themselves prove permanent membership, dissolution, merger, or physical
transformation. The publication keeps all three link types on the dated E13:

- `stm:reportedComponentOrganization` for `has_parts*_id`;
- `stm:reportedCompositeOrganization` for `part_of_id`;
- `stm:reportedOwnerOrganization` for `owned_by_id` and `owned_by_id2`.
...
flowchart TB
    ...
    OBS["E13 dated Almanakken observation"]
    OBS -->|"reported component"| SPLIT1
    OBS -->|"reported component"| SPLIT2
    OBS -->|"assigned to"| MAIN
    OBS -->|"reported composite"| PARENT
    OBS -->|"reported owner"| REF

    classDef crm_E39_Actor fill:#ffbdca,stroke:#000000,color:#000000
    class SPLIT1,SPLIT2,MAIN,PARENT,REF crm_E39_Actor
```

### Component and composite evidence

Surinamese plantation history is full of mergers. Small coffee plantations were absorbed by larger sugar plantations as the coffee economy declined. Almanakken v2 records structural references in the `has_parts` columns:

- `has_parts1_lab` / `has_parts1_id` through `has_parts4_lab` / `has_parts4_id` record referenced component plantations.

When `has_parts1_id = Q59115689` occurs on the row for
`plantation_id = Q4392658`, the publication records:

```
E13 row --P140 assigned attribute to--> E74 Q4392658 (Geijersvlijt)
E13 row --stm:reportedComponentOrganization--> E74 Q59115689 (Heystvliet)
```

The label and original reference text stay on the observation. Researchers can
later reconcile this into E66/E68/E81 events if other evidence establishes
what changed, when it changed, and whether the organization, physical
plantation, or both were affected.

The `part_of_lab` / `part_of_id` columns use the same evidence pattern:

```
E13 row --stm:reportedCompositeOrganization--> E74 (part_of_id)
```

### Reported ownership

Some almanac entries indicate that one plantation organization is owned by another. The `owned_by_id` / `owned_by_id2` / `owned_by_lab` columns capture this:

```
E13 observation --stm:reportedOwnerOrganization--> E74 (owned_by_id)
```

This is ownership reported for the E74 organization in one dated row. It is not
a P51/P52 claim that the referenced organization owned the E25 land. It also
remains distinct from the verbatim owner-name transcription, which has not yet
been reconciled to a person or organization authority record.
...
flowchart LR
    ...
    E25 -->|"stm:hasOrganizationalAssociation"| E74_ORG
    E74_ORG -->|"stm:associatedPhysicalPlantation"| E25
...
The Layer 10 reference network makes the possible connection visible without
claiming a dissolution chain that the row alone cannot establish:

```
E13 row for Q4392658 --stm:reportedComponentOrganization--> E74 Q131349015
E74 Q4392658 --stm:psurId--> "PSUR0118"
E74 Q131349015 --stm:psurId--> "PSUR02xx" (if assigned)
```

A reviewed follow-up assertion may add an E68 dissolution or E81
transformation, with its own source and time-span.
...
flowchart TB
    ...
    subgraph "Observation Pattern (temporal)"
        ...
        'Abraham de Leon'
        role: owner"]
        SPLIT["E74 referenced component"]

        OBS -->|"P140 assigned attribute to"| E74_ORG
        OBS -->|"P14 carried out by<br/>(P14.1 role: picot:owner)"| PO
        OBS -->|"reported component organization"| SPLIT
    end
...
- The plantation (E25) has a reciprocal researched association with it
...
- Dated ownership and component/composite references connect other E74 records
