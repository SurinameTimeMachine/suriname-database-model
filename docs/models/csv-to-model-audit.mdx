...
flowchart LR
    ...
    E24["E24 Plantation
    stm:plantation/nieuw-java"]
    E53["E53 Place
    stm:place/1930/fid-1572"]
...
> **Model target:** `E53 Place` --- `stm:fid` (literal integer); used in URI `stm:place/1930/fid-{fid}`
...
```
E24 stm:plantation/{slug} --P52 has current owner--> E74 wd:{Q-ID}
```
...
> **Model target:** `E74 Organization` --- `stm:absorbedInto` (merger network); and/or `P51 has former or current owner`
...
2. An organization that was absorbed into the primary (`stm:absorbedInto`)

The SKILL.md mapping says: _"P51 former owner, stm:absorbedInto qid"_. This is slightly ambiguous --- should every `qid_alt` produce both a P51 triple and an `stm:absorbedInto` triple? Or is context needed to determine which?

**Recommendation:** Treat `qid_alt` as `stm:absorbedInto` by default (the alternative organization was absorbed into the primary). The P51 triple is then inferred: if an organization was absorbed, the plantation had a former owner. This avoids generating P51 triples without temporal qualification.
...
| `qid_alt`          | E74 stm:absorbedInto  | 14%      | Works (clarify P51 vs absorbedInto)         |
...
The Almanakken is the time-series backbone --- 23,000 annual snapshots of plantation organizations. Each row becomes an `stm:OrganizationObservation`. The columns below are those marked "Crucial for Linking" or "Primary Information: Yes" in the [data-sources documentation](../data-sources/06-almanakken.md).
...
flowchart TB
    ...
    OBS -->|"stm:observationOf"| E74
    OBS -->|"stm:observationYear"| YEAR["1818"]
    OBS -->|"prov:hadPrimarySource"| E22
    E74 -->|"P1 identified by"| E41
    OBS -->|"stm:hasOwner"| PERSON
...
> **Model target:** Observation URI --- `stm:obs/{recordid}`
...
> **Model target:** `stm:OrganizationObservation` --- `stm:observationYear` (xsd:gYear)
> **Coverage:** 23,003 / 23,003 (100%)
...
> **Model target:** `stm:OrganizationObservation` --- `stm:observationOf` --> `wd:{Q-ID}` (E74 Organization)
...
```
Almanakken: stm:obs/1818-28-1 --stm:observationOf--> wd:Q59116947
QGIS:      stm:plantation/leonsberg --P52 has current owner--> wd:Q59116947
```
...
> **Sample:** `28`, `56`, `81`
> **Model target:** `stm:OrganizationObservation` --- `stm:pageReference` (string)
> **Coverage:** 22,999 / 23,003 (100%)

**Verdict: Works.** Page reference for provenance. Currently a convenience string. As TvO notes, DBNL does not yet provide IIIF, but if it does in the future, this could become a IIIF canvas URI. The model's `stm:pageReference` is intentionally untyped to accommodate both.
...
> **Model target:** `stm:OrganizationObservation` --- `stm:hasOwner` --> `PersonObservation` (picot:owner role)
> **Coverage:** 13,049 / 23,003 (57%)

**Verdict: Works with caveat.** The owner field contains one or more person names in a single string, sometimes comma-separated, sometimes with punctuation quirks (note the period after "Monsanto."). Each name becomes a `PersonObservation` with `pico:hasRole picot:owner`, linked to the `stm:OrganizationObservation` for that year.
...
> **Model target:** `stm:OrganizationObservation` --- `stm:hasAdministrator` --> `PersonObservation` (picot:administrator role)
...
**Issue to flag:** The 9% coverage for `administrateurs` combined with 71% for `admin_europa` (16,452 rows) and 26% for `admin_suriname` (5,943 rows) suggests the data is split across three columns by era. The model (SKILL.md) only maps `administrateurs` to `stm:hasAdministrator`. The Europa/Suriname variants are not in the mapping table.

**Recommendation:** Add `administrateurs_in_Europa` and `administrateurs_in_Suriname` as additional sources for the `stm:hasAdministrator` --> `PersonObservation` mapping. Tag the PersonObservation with a location qualifier (Europe vs Suriname) to preserve the distinction.
...
> **Model target:** `stm:OrganizationObservation` --- `stm:hasDirector` --> `PersonObservation` (picot:director role)
...
> **Model target:** `stm:OrganizationObservation` --- `stm:enslavedCount` (xsd:integer)
...
**Recommendation for v2.0:** Add `stm:sharedWorkforceWith` property linking to a second E74, to model the shared-enslaved-population relationship. This is semantically different from `stm:absorbedInto` (a merger) --- it describes an operational arrangement, not an identity change.
...
> **Model target:** `stm:OrganizationObservation` --- `stm:hasProduct` (string or SKOS concept)
...
> **Model target:** `stm:OrganizationObservation` --- `stm:locationStd` (string)
...
> **Model target:** `stm:OrganizationObservation` --- `stm:isDeserted` (boolean or `E55 Type` PlantationStatus)
> **Coverage:** 2,294 / 23,003 (10%)

**Verdict: Works with caveat.** The model maps this to `stm:isDeserted` as a boolean, but the actual CSV values are Dutch text strings (`verlaten`, `vervallen`), not true/false. Two options:

1. **Boolean coercion:** Treat any non-empty value as `true`, empty as `false` (or unknown).
2. **E55 Type mapping:** Map the Dutch terms to `stm:PlantationStatus_Abandoned` vocabulary terms, preserving the distinction between `verlaten` (abandoned) and `vervallen` (dilapidated).
...
> **Model target:** `stm:OrganizationObservation` --- `stm:sizeAkkers` (xsd:integer)
...
> **Model target:** `E74 Organization` --- `stm:absorbedInto` targeting the primary `plantation_id`
...
The model maps `split_id` values to `stm:absorbedInto`:

```
wd:Q-split1 --stm:absorbedInto--> wd:Q-primary
```
...
1. When `split_id` is present: emit `stm:absorbedInto` triple as designed.
2. When only `split_lab` is present: create an E41 Appellation with the label, attached to the observation as `stm:absorbedPartLabel` (new property), flagged for resolution.
...
> **Model target:** `stm:OrganizationObservation` --- `stm:referencedBy` --> `wd:{Q-ID}`
> **Coverage:** reference_std_id: 3,205 / 23,003 (14%)

**Verdict: Works with caveat.** TvO explains this is _"a plantation that owns the plantation mentioned in this record"_ --- a form of inter-plantation ownership. The model uses `stm:referencedBy` which is admittedly vague.

**Recommendation:** Consider a more specific property, such as `stm:ownedByPlantation`, to distinguish this from generic references. The relationship is directional and ownership-based, which `stm:referencedBy` does not capture.
...
> **Model target:** `stm:OrganizationObservation` --- `stm:function` (free text, deferred priority)
...
> **Model target:** `stm:OrganizationObservation` --- `stm:additionalInfo` (free text, deferred priority)
...
**Must add to model.** Map both to `stm:hasAdministrator` PersonObservation, with an additional qualifier for geographic scope (Europe/Suriname).
...
| `year`                    | stm:observationYear          | 100%     | Works                                             |
| `plantation_id`           | E74 via stm:observationOf    | 99.97%   | Works                                             |
...
| `page`                    | stm:pageReference            | 100%     | Works                                             |
...
| `slaven`                  | stm:enslavedCount            | 7%       | Works (v2.0 adds slaves_norm)                     |
| `namen_totslaafgemaakten` | E41 Appellation (srn)        | 14%      | Works (PROMINENT placement)                       |
| `product_std`             | stm:hasProduct               | 69%      | Works                                             |
| `loc_std`                 | stm:locationStd              | 100%     | Works                                             |
| `deserted`                | stm:isDeserted / E55 Type    | 10%      | Needs adjustment (text, not boolean)              |
| `size_std`                | stm:sizeAkkers               | 19%      | Works                                             |
| `split1_id`               | stm:absorbedInto             | 3%       | Needs adjustment (label coverage 34%, ID only 3%) |
| `partof_lab/id`           | P107i is member of           | 0.4%     | Works                                             |
| `reference_std_id`        | stm:referencedBy             | 14%      | Works (consider renaming property)                |
| `function`                | stm:function                 | 58%      | Works                                             |
| `additional_info`         | stm:additionalInfo           | 8%       | Works                                             |
...
1. **Add `administrateurs_in_Europa` and `administrateurs_in_Suriname`** to the SKILL.md mapping table, mapping both to `stm:hasAdministrator` PersonObservation with a geographic qualifier.
...
3. **Handle unresolved `split_lab` references** --- create a fallback pattern for the 7,035 rows where a merged-component name exists but no Q-ID. Consider `stm:absorbedPartLabel` as an interim property.
...
5. **Consider renaming `stm:referencedBy`** to something more specific like `stm:ownedByPlantation` to capture the directional ownership relationship.

6. **Plan for v2.0 columns** --- `slaves_norm`, `slaves_shared_with`, `sranantongo_naam`. The `slaves_shared_with` column introduces a new structural relationship (`stm:sharedWorkforceWith`?) not yet in the model.
