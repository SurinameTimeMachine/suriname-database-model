...
/** Derived Concordans address link attached to a historical-address record. */
export interface HistoricalAddressLink {
  id: string;
  sourceRow: string | null;
  source: string;
  key1885: string | null;
  certainty: 'certain' | 'probable' | 'unresolved';
  certaintyByPlace?: Record<string, 'certain' | 'probable' | 'unresolved'>;
...
/** Registry-backed attribution for a Concordans address history. */
...
export interface GazetteerPlace extends TombstoneFields {
  ...
  /** Derived Concordans address links (historical-address records only).
   * Regenerated by link-concordans-1885-places.ts; never hand-edited or
   * POSTed back — the editor strips it from save payloads. */
