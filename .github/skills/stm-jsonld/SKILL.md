...
- Use the E53 location as the persistent point anchor, typed with standard
  `crm:E53_Place` and `geo:Feature`, following the HisGIS / Amsterdam Time
  Machine space-time-prism pattern. Attach each dated address statement to that
  anchor with `crm:P140_assigned_attribute_to`, assign the dated address
  appellation with `crm:P141_assigned`, and serialize point anchors as
  GeoSPARQL `POINT` WKT. Only create a cross-map address concordance when both
  source systems and their spatial correspondence are evidenced.
